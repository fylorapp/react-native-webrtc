...
TypedArrayBase::TypedArrayBase(jsi::Runtime &runtime, size_t size,
                               TypedArrayKind kind)
    : TypedArrayBase(
          runtime,
          runtime.global()
...
uint8_t* getByteArray(jsi::Runtime &runtime, jsi::Object *jsObj) {
  if (!jsObj->isArrayBuffer(runtime)) {
    throw std::runtime_error("Object is not an ArrayBuffer");
  }
  auto jsArrayBuffer = jsObj->getArrayBuffer(runtime);
...
template <TypedArrayKind T>
TypedArray<T>::TypedArray(jsi::Runtime &runtime, size_t size)
    : TypedArrayBase(runtime, size, T){};

template <TypedArrayKind T>
TypedArray<T>::TypedArray(jsi::Runtime &runtime,
...
template <TypedArrayKind T>
TypedArray<T>::TypedArray(TypedArrayBase &&base)
    : TypedArrayBase(std::move(base)) {}
...
template <TypedArrayKind T>
void TypedArray<T>::update(jsi::Runtime &runtime,
...
const jsi::PropNameID &PropNameIDCache::getConstructorNameProp(
    jsi::Runtime &runtime, TypedArrayKind kind) {
  switch (kind) {
